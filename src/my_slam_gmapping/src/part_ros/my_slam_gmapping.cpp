...
{
    sst_ = node_.advertise<nav_msgs::OccupancyGrid>("map", 1000, true);//iiiiii
    sstm_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata", 1000, true);//iiiiiii
    ss_ = node_.advertiseService("dynamic_map", &MySlamGMapping::mapCallback, this);

    {
        //用message_filters来订阅scan_topic_，进而初始化scan_filter_，
        scan_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, scan_topic_, 5000);//iiiiii
        //tf::MessageFilter，订阅激光数据同时和odom_frame之间的转换同步
        scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*scan_filter_sub_, tf_, odom_frame_, 5000);//iiiiii
